...
func (r *CouchbaseRepository) InsertTransactionSaga(saga models.TransactionSaga) error {
	col := r.Bucket.Scope("_default").Collection("pay-saga")

	_, err := col.Upsert(saga.SagaID, saga, nil)
	if err != nil {
		return err
	}
...
func (r *CouchbaseRepository) QueryTransactionSaga(transactionID string) ([]models.TransactionSaga, error) {
	queryResult, err := r.Cluster.Query(
		fmt.Sprintf("SELECT p.* FROM `pay.saga`.`_default`.`pay-saga` p WHERE p.transactionID='%s'", transactionID), nil)
	if err != nil {
		return nil, err
	}
